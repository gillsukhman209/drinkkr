...
struct OnboardingCompleteView: View {
    ...
    var body: some View {
        VStack(spacing: isCompact ? 30 : 40) {
            ...
            VStack(spacing: 20) {
                ...
                Text("Your personalized journey to freedom begins now. Remember, every day without alcohol is a victory worth celebrating.")
                    .font(.system(size: isCompact ? 16 : 18, weight: .medium))
                    .foregroundColor(ColorTheme.textSecondary)
                    .multilineTextAlignment(.center)
                    .lineSpacing(4)
            ...
            VStack(spacing: 16) {
                summaryCard(
                    title: "Your Goals",
                    value: "\(viewModel.selectedGoals.count) selected",
                    icon: "target",
                ...
                summaryCard(
                    title: "Quit Date",
                    value: formatDate(viewModel.userProfile.quitDate),
                    icon: "calendar",
                ...
                summaryCard(
                    title: "Check-in Time",
                    value: formatTime(viewModel.userProfile.checkInTime),
                    icon: "clock",
            ...
            VStack(spacing: 12) {
                Text("\"The journey of a thousand miles begins with one step.\"")
                    .font(.system(size: isCompact ? 14 : 16).italic())
                    .foregroundColor(ColorTheme.accentCyan)
                    .multilineTextAlignment(.center)
                
                Text("You've taken that step. Now let's walk together.")
                    .font(.system(size: isCompact ? 14 : 16, weight: .medium))
                    .foregroundColor(ColorTheme.textSecondary)
            }
            .padding(.horizontal, 40)
    ...
    private func confettiPiece(index: Int) -> some View {
        let colors: [Color] = [ColorTheme.accentCyan, ColorTheme.accentPurple, ColorTheme.successGreen, ColorTheme.accentPink]
        ...
        return RoundedRectangle(cornerRadius: 2)
            ...
            .opacity(confettiPhase >= 2 ? 1.0 : 0.0)
            .scaleEffect(confettiPhase >= 2 ? 1.0 : 0.0)
            .animation(
    ...
    private func startSuccessAnimation() {
        // Phase 1: Main elements appear
        withAnimation(.spring(response: 0.8, dampingFraction: 0.8).delay(0.2)) {
            isVisible = true
        }
        
        // Phase 2: Confetti appears
